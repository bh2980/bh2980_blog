...
이 구조를 만들고 나니 비로소 모든 흐름이 한 줄로 이어졌습니다. 에디터에서 지정한 인라인 블럭은 주석으로 저장되고, 저장된 주석은 다시 annotation 데이터로 파싱되며, 최종적으로는 Shiki의 decorations와 transformer를 통해 실제 코드 블록 UI로 렌더링되었습니다.

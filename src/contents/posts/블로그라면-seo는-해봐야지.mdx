...
robots.txt는 **검색 엔진 크롤러가 어떤 경로를 크롤링 할 수 있는지 제어하는 역할**을 합니다. 사이트의 루트 경로(/robots.txt)에 위치하고, 크롤러는 이 파일을 먼저 확인한 뒤 페이지를 탐색합니다.&#x20;
...
Next.js(App Router)에서는 `src/app` 디렉터리에 `robots.txt` 또는 `robots.ts` 파일을 추가하면, 빌드 결과로 사이트 루트(/robots.txt)에 해당 파일이 생성됩니다.

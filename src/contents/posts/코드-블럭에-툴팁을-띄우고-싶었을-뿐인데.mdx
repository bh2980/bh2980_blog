...
먼저 remark 단계에서는 code fence 안에 들어 있는 주석을 읽어 중간 포맷으로 파싱한 뒤, 이를 다시 Shiki가 이해할 수 있는 payload로 바꿨습니다. 이 단계에서 문자 범위 annotation은 `decorations`로, 줄 단위 annotation은 `lineDecorations`나 `rowWrappers` 같은 형태로 정리되었습니다.

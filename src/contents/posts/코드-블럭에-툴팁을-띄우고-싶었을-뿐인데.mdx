...
이 간극을 메우기 위해 저는 Keystatic의 내부 깊숙한 곳을 건드려야 했습니다. 기본 코드 블록 컴포넌트는 애초에 이런 '인라인 블록' 편집 모델을 상정하고 있지 않았기 때문입니다. 그래서 에디터가 MDX를 파싱하거나 직렬화하는 결정적인 순간에 제 로직을 끼워 넣을 수 있도록 `@keystatic/core`를 패치하여 두 가지 '훅(Hook)' 지점을 확보했습니다.
...
확보한 훅을 통해 이제 각 단계에서 원하는 변환을 끼워 넣을 수 있게 되었습니다. 단순히 텍스트를 갈아끼우는 것이 아니라, **코드와 Annotation이 결합된 '하나의 데이터 구조'**&#xB85C; 취급하는 것이 핵심입니다.

이를 위해 저는 주석에 담긴 정보를 분석해 중간 표현(Intermediate Representation)을 먼저 설계했습니다. 텍스트 형태의 주석을 에디터가 이해할 수 있는 트리 구조(AST)의 노드로 변환하는 과정에서, 정보가 누락되거나 순서가 뒤섞이지 않도록 '튼튼한 설계도'를 만든 셈입니다.

...
Next.js는 페이지를 트리 형태로 관리합니다. 최상위 레이아웃부터 상세 페이지, 그리고 그 안의 작은 컴포넌트들까지 하나의 트리를 구성합니다. 그런데 이 트리 중 어느 한 곳에서라도 동적 요소가 발견된다면, Next.js는 그 지점부터 아래로 이어지는 모든 경로를 동적 페이지로 판정해 버립니다.

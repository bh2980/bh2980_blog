...
저는 글 본문 페이지에 `/posts/[slug]` 형태의 라우터를 사용하기 때문에 글 본문의 페이지 파일에서 정적으로 메타데이터를 결정할 수 없습니다. 이럴 경우에 `metadata` 객체가 아니라 `generateMetadata`를 사용하면 각 게시글의 정보를 불러와 동적으로 메타 데이터를 생성할 수 있습니다.

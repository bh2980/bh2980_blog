...
| ④ | ~~이관 무결성 대조~~ → **이관 자체가 미실행**(운영 DB entries 8건 전부 draft, content_addresses 0건) |
...
## 5.1 운영 DB 실측 (읽기 전용, 2026-09-22)

`CMS_DATABASE_URL`로 실제 DB를 읽기만 해서 확인한 결과다(SELECT만 실행).

| 항목 | 값 |
| --- | --- |
| `entries` | **8건 — 전부 `draft`** (post 5 · memo 1 · tag 1 draft + 1 published) |
| `content_addresses` | **0건** |
| `media_assets` | **0건** |
| `entry_bodies` / `entry_references` / `folders` | 9 / 1 / 1 |
| `schedules` | 0 |
| 공개 노출되는 current 주소 | **0건** |

**해석: 이관이 아직 실행되지 않았다.** `cms_migrations` 테이블과 M6 스키마는 있지만 레거시 49편·이미지 22장·카테고리/태그가 들어오지 않았다. 같은 DB에 Payload CMS 테이블(`payload_*`, `posts`, `memos`, `media`, `users`)이 함께 있어 **공유/레거시 DB**다.

**전환 차단 사유가 하나 더 늘었다(최상위).** 플래그를 켜기 전에:

1. 파일→DB 이관 실행(`migrate-from-files` apply): 글·메모 49편, 카테고리·태그 레코드, 이미지 22장.
2. **카테고리를 published로 넣어야 한다.** `toPost`는 해석 가능한 published 카테고리가 없는 글을 공개하지 않는다(`repositories/postgres.ts:88-92`) — 통합 테스트로 고정했다. 카테고리 없이 이관하면 49편이 전부 조용히 사라진다.
3. 이관 후 49주소 대조(부록 D SQL)와 이미지 22장 R2 키·체크섬 대조.
4. 그 뒤에 `CMS_PUBLIC_REPOSITORY=postgres` + 재배포.

이관은 운영 DB 쓰기라 **사용자 승인 없이 하지 않는다**(계획 원칙 2).
...
1. **DB 전환 승인**: (a) 승인 (b) 조건부 승인 (§5.1 이관 실행 + §6 미리보기 경로 + 실측을 조건으로) (c) 보류
2. **이관 실행 승인**: 레거시 49편·이미지 22장·카테고리/태그를 운영 DB로 넣는 작업을 지금 진행할지. **카테고리 published 여부가 공개 노출을 좌우한다.**

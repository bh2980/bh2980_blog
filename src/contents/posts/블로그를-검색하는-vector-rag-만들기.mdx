...
저는 이 Vector DB를 사용하기 위해 Supabase를 선택했는데요. 별도의 벡터 DB를 운영하는 것보다 RAG 서비스 자체에 집중하고 싶었고, Supabase는 pgvector를 지원하기 때문에 추가 인프라 없이 바로 시작할 수 있었거든요. 그리고 가장 중요한 점으로, 무료이기도 했습니다.
...
이제 문서를 모두 적재했으니 사용자가 쿼리를 입력하고, 답변을 받아오는 코드를 작성해야 했습니다. 제 블로그는 Next.js를 사용 중이라 간단하게 API Handler를 사용해 구현했습니다.

쿼리 파이프라인에서 눈여겨볼 부분은 질문을 그대로 임베딩하지 않는다는 점입니다. 먼저 이전 대화 맥락을 반영해 벡터 검색에 최적화된 형태로 재작성하는 과정을 거칩니다.
...
그리고 재작성된 쿼리를 활용해 유사한 청크를 검색해 가져옵니다. 이때 모델마다 문서 임베딩용, 그리고 쿼리 작성용으로 전달해줘야하는 값이 다를 수 있습니다. qwen/qwen3-embedding-8b 모델의 경우는 쿼리 작성 시에는 Instruct: \~를 활용해서 임베딩하라고 하더라구요.
...
![](/assets/images/posts/%EB%B8%94%EB%A1%9C%EA%B7%B8%EB%A5%BC-%EA%B2%80%EC%83%89%ED%95%98%EB%8A%94-vector-rag-%EB%A7%8C%EB%93%A4%EA%B8%B0/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202026-04-08%20%EC%98%A4%ED%9B%84%208.13.01.png)
...
그런데 한 가지 의문이 생겼습니다. 일단은 간단하게 만들려고 문서 전체를 하나의 청크로 임베딩했는데, 만약 청크 단위를 좀 더 작게 세분화한다면 성능이 더 좋아질까요? 그렇다면 성능은 어떻게 측정하는걸까요? 그래서 다음에는 이 성능 측정에 관한 이야기를 해보려고 합니다.

...
<Collapsible>
  ...
  1. `T extends [infer A, ...infer Rest]` : T가 1개 이상의 요소를 가진 배열인지 검사하고, 맞을 경우 첫 번째 요소의 타입을 A로, 나머지를 Rest 타입으로 추출한다.
  ...
  위는 정답으로 인정되나 튜플이 아닌 일반 배열일 경우 정확한 길이를 알 수 없기 때문에 빈 배열이 들어와도 string을 return한다. 반면 infer를 사용할 경우, 첫 번째 요소의 존재 검증을 하기 때문에 올바른 정답이 나온다.
</Collapsible>

...
앞서 `metadataBase`를 설정해두었기에 상대 경로로 URL을 작성한 모습입니다. 위와 같이 설정하면 `/posts` 페이지를 기준으로 `<link rel="canonical">` 태그가 자동으로 생성되며, `/posts?category=development`와 같은 쿼리 스트링이 붙은 URL들은 검색엔진 관점에서 모두 `/posts`의 변형으로 인식되어 색인과 평가 기준이 하나의 URL로 통합됩니다.

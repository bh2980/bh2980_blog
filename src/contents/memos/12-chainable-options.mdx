...
<Collapsible>
  ```ts
  type Chainable<S extends object = {}> = {
  // @mark Tooltip {32-66} content="key가 이미 있는 key일 경우 never로 만들어 타입 에러를 띄웁니다."
    option: <K extends string, V>(key: K extends keyof S ? never : K, value: V) => Chainable<Omit<S, K> & Record<K, V>>
    get: () => S
  }
  ```

  key가 이미 있는 key일 경우 never로 만들어 타입 에러를 띄웁니다.
</Collapsible>

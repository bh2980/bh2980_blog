...
이 둘의 차이는 메타 데이터가 정적인지 동적인지의 차이입니다. 정적 string만을 사용하는 `metadata`와 달리 `generateMetadata`의 경우 `getPost` 함수를 통해 타이틀(title)과 요약(excerpt)를 가져와 metadata를 생성하고 있습니다. 저는 /posts/\[slug] 형태의 라우

만약 메타 데이터를 설정하지 않은 페이지는 기본적으로 상위 라우트의 메타 데이터를 상속받습니다.

...
export const metadata: Metadata = {
	...
	description: "개발하면서 배운 것들과 경험을 기록합니다.",
	// @line plus
	// @char Tooltip {26-33} content="앞서 layout.tsx의 메타 데이터에서 metadataBase를 지정해두었기에 상대 경로 사용 가능"
	alternates: { canonical: `/posts` },
};
```

위와 같이 설정하면 `/posts` 페이지를 기준으로 `<link rel="canonical">` 태그가 자동으로 생성되며, `/posts?category=development`와 같은 쿼리 스트링이 붙은 URL들은 검색엔진 관점에서 모두 `/posts`의 변형으로 처리됩니다.

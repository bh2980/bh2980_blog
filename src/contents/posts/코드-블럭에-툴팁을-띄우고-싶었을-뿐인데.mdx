...
이를 위해 각 파싱/직렬화 단계 직전에 제 훅을 끼워넣는 패치를 통해 파일이 에디터로 들어오기 직전(`afterMarkdownParse`), 그리고 에디터에서 파일로 나가기 직전(`beforeSerialize`)이라는 두 지점을 확보했습니다.
...
여기까지 하니 에디터와 파일 간의 통신은 이루어진 상태였습니다. 이제 마지막으로 파일과 실제 게시글 코드블럭 사이를 이어주는 것이 남아있었습니다. 다행히도 이 부분엔 사용할 라이브러리가 있었습니다. 바로 Shiki의 decorations입니다.

처음부터 제가 이 시스템을 구상할 때 힌트로 삼았던 것도 결국 이 지점이었습니다. 주석은 파일 안에 저장되는 직렬화 포맷일 뿐이고, 실제 화면에서는 이를 다시 렌더링 가능한 정보로 번역해줘야 했습니다. 저는 이 작업을 곧바로 JSX를 만드는 식으로 처리하지 않고, remark와 rehype 플러그인 사이에 한 단계씩 나누어 연결하는 방식을 택했습니다.

먼저 remark 단계에서는 code fence 안에 들어 있는 주석을 읽어 중간 표현으로 파싱한 뒤, 이를 다시 Shiki가 이해할 수 있는 payload로 바꿨습니다. 이 단계에서 문자 범위 annotation은 decorations로, 줄 단위 annotation은 lineDecorations나 rowWrappers 같은 형태로 정리되었습니다.
...
이 지점에 이르러서야 전체 구조가 비로소 하나로 닫히기 시작했습니다. 에디터에서 만든 의미는 주석으로 저장되고, 저장된 주석은 다시 annotation 데이터로 파싱되며, 최종적으로는 게시글 렌더링까지 이어졌습니다. 처음에는 그저 `Tooltip`과 `strong` 정도만 편하게 쓰고 싶었을 뿐인데, 결과적으로는 코드 블록 전체를 관통하는 annotation 파이프라인을 갖게 된 셈입니다.

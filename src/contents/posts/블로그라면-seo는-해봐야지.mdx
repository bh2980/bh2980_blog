...
동적 방식으로 설정할 경우 컴포넌트 코드처럼 작성할 수 있다는 특징이 있습니다. 하지만 모든 style 문법을 지원하는 것은 아니므로 스타일이 적용되는지 유의하며 사용해야 합니다.

<Callout variant="info">
  Image 함수 내에서 tailwind 문법을 사용하려면 className이 아닌 [tw 속성](https://vercel.com/kb/guide/using-tailwind)을 사용해야 합니다. 이 역시 모든 속성이 지원되는 것이 아니므로 유의하여 사용하여야 합니다.
</Callout>

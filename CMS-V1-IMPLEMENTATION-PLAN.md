...
| M4-TW-1 | TODO | TW | — | — | 일괄·복제 실패 테스트 |
| M4-BE-1 | TODO | BE | — | — | 일괄 작업 API |
| M4-FE-2 | TODO | FE | — | — | 목록 일괄 메뉴 (에디터 템플릿 버튼은 M5-FE-1로 분리) |
| M4-RV-1 | TODO | RV | — | — | — |
| M5-BE-1 | TODO | BE | — | — | 복제와 템플릿 생성 (ex-M4-BE-2) |
| M5-FE-1 | TODO | FE | — | — | 에디터 툴바 템플릿 버튼 (M4-FE-2에서 분리) |
| M5-ED-1 | TODO | ED | — | — | 필드·블록 확장 예제 (ex-M4-ED-1) |
| M5-FE-2 | TODO | FE | — | — | 관계 선택기와 record 폼 (갭 분석 반영) |
| M5-RV-1 | TODO | RV | — | — | — |
...
| M5 | 복제·템플릿·저작 확장 | §12.1-4 | 복제·코드 정의 템플릿 3종·필드/블록 확장 예제·관계 선택기와 record 폼이 저장·재열기를 보존한다 |
...
종료 조건: F13 복제·템플릿, F07 확장 예제, §6.1 관계 선택기와 §5.2 record 폼. 관심사는 저작 확장 하나로 한정한다.

### M5-BE-1 복제와 템플릿 생성 (ex-M4-BE-2)

- **목적:** F13.
- **주요 내용:** `POST /entries/:id/duplicate`, `POST /entries`의 `templateId`. 새 ID, slug 빈 값, 상태/일정/공개본/날짜 비복사, 같은 폴더, 미디어 재업로드 없음. 템플릿은 코드 정의만(일반 게시글, 알고리즘 풀이 메모, Type Challenge 풀이 메모). 템플릿 목록은 `/meta`에 노출.
- **선행:** M4-TW-1
- **담당:** BE
- **완료 조건:** 복제 글이 초안이고 원본 공개본과 무관.
- **검증:** API 테스트

### M5-FE-1 에디터 툴바 템플릿 버튼 (M4-FE-2에서 분리)

- **주요 내용:** 에디터 상단 툴바에 `템플릿` 버튼. 드롭다운에서 3종 템플릿 선택 시 확인 다이얼로그("현재 내용이 교체됩니다") 후 본문·필드에 적용, 자동저장으로 persisted. 템플릿 목록은 `/meta`에서 조회.
- **선행:** M5-BE-1, M3-FE-1
- **담당:** FE
- **완료 조건:** 템플릿 적용 후 저장·재열기 시 내용 유지. 확인 없이 기존 초안을 덮어쓰지 않음.
- **검증:** 브라우저

### M5-ED-1 필드·블록 확장 예제 (F07, ex-M4-ED-1)
...
- **담당:** RV
- **선행:** M5-BE-1, M5-FE-1, M5-ED-1, M5-FE-2

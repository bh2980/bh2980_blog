...
개인 블로그의 콘텐츠를 웹에서 작성·정리·발행하는 1인용 CMS를 만든다. Keystatic의 본문 중심 디자인, 입력 즉시 서식이 적용되는 편집, MDX 저장, 관계 필드, 커스텀 컴포넌트의 장점을 유지한다. CMS 내부를 패치하지 않고 필드·본문 블록·저장소를 확장할 수 있어야 한다. v1의 최종 완료 상태에서는 Keystatic을 실행 경로와 개발 환경에서 완전히 제거한다.
...
| 교체 완료 | 운영 전환 검수 후 Keystatic 패키지·설정·라우트·전용 코드·패치 제거 |
...
- 콘텐츠 최대 본문 2MiB, JSON 메타데이터 256KiB, 제목 200자, slug 200자를 기본으로 한다. 문자 수 제한(예: 200자)은 결정론적 서버 동작을 위해 유니코드 코드 포인트(Unicode code points)를 기준으로 계산한다. 서버 설정과 API 메타데이터에 같은 제한을 표시한다.
- 발행 시 대상 유효성 재검사는 M3 단계에서 트랜잭션으로 처리하며, M2 단계는 순수한 검증 및 준비 과정만 제공한다.
...
새로운 CMS 메타데이터 통신 계약(wire keys)은 `title`, `summary`, `categoryId`, `tagIds`, `publishedAt`, `policy`, `itemIds`다. 컬렉션별 허용 목록은 유지하되, 기존 Keystatic 이름은 마이그레이션 입력으로만 사용하며 새로운 통신 계약에는 포함하지 않는다. 컬렉션 항목 중 `itemIds`는 게시글(post)만을 대상으로 한다.
...
- 동시 저장 충돌은 409로 반환하고 현재 서버 번호를 전달한다. 오래된 탭이나 요청이 최신 내용을 덮어쓰지 않는다. 안정적인 ContentStore 에러 코드는 컬렉션과 slug 고유성 충돌의 경우 `slug_conflict`다. 서비스는 PostgreSQL 메시지를 직접 파싱해서는 안 된다.
...
9. **Keystatic 완전 제거:** 기능 동등성과 운영 확인을 마친 뒤 Keystatic 패키지와 lockfile 항목, 설정, 관리자 라우트, 전용 필드·변환 코드, 타입·빌드 설정, patch-package 패치와 사용하지 않는 생성물을 제거한다. 기존 공개 렌더링에 계속 필요한 범용 MDX 코드와 콘텐츠·이미지는 CMS 중립 위치로 옮겨 보존한다. 저장소 검색, 설치, 빌드와 테스트로 실행·개발 의존성이 남지 않았음을 확인한다.
...
| Keystatic 제거 | 패키지·lockfile·설정·라우트·import·전용 패치와 실행 코드가 남아 있지 않다. 새 CMS의 작성·발행·공개 조회·빌드·테스트가 Keystatic 없이 동작하고 기존 콘텐츠와 공개 주소가 유지된다 |
...
각 구현 작업은 담당자가 아닌 Reviewer가 명세 적합성, 기존 동작 회귀, 보안·데이터 손실 가능성과 테스트 근거를 확인한 뒤 통합한다. Reviewer는 같은 기능을 다시 구현하지 않고 구체적인 파일·위치·재현 조건을 근거로 승인 또는 수정 요청을 반환한다. Keystatic 제거와 운영 전환은 전체 변경을 대상으로 별도 최종 리뷰를 수행한다.

...
이 주석을 다시 '편집 가능한 구조'로 되살려내는 유일한 방법은, 에디터와 파일이 서로 다른 언어를 쓰고 있음을 인정하는 것이었습니다. 에디터에게는 Tooltip이나 strong을 직접 핸들링할 수 있는 트리(Tree) 구조의 객체가 필요했지만, 마크다운 파일은 그저 텍스트 데이터만을 원했으니까요.

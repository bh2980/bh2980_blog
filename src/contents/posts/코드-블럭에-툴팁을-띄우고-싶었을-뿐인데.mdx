...
publishedDateTimeISO: 2026-03-21T20:51:00.000Z
tags:
  - blog
policy:
  discriminant: normal
...
그래서 처음에는 간단하게 `PureMdx`라는 커스텀 컴포넌트를 도입해 해결했습니다. 컴포넌트의 속성은 정규화 대상이 아니었거든요. 하지만 또 다른 문제가 발생했는데, 컴포넌트를 사용할 경우 복잡한 렌더링 파이프라인을 거쳐야한다는 점이었습니다.

```mermaid
flowchart TD
    A["에디터에서 원본 작성"] --> B["일반 MDX처럼 바로 렌더링 불가"]
    B --> C["PureMdx에 텍스트로 보관"]
    C --> D["내부 본문을 다시 MDX로 재파싱"]
    D --> E["Code Hike 플러그인 적용"]
    E --> F["최종 코드 블록 렌더링"]

    style C fill:#fff4dd,stroke:#333,color:#000
    style D fill:#fff1f1,stroke:#ff9494,stroke-width:2px,stroke-dasharray: 5 5,color:#000
```
...
제가 정말 풀고 싶었던 문제는 심플했습니다. 코드 블록 안의 특정 단어를 드래그해서 밑줄을 적용하거나 툴팁을 띄우는, 아주 기본적인 에디터의 기능을 코드블럭에 사용하고 싶었을 뿐이죠.&#x20;

그렇다고 이걸 위해 코드 본문 전체를 복잡한 MDX 컴포넌트 문법으로 감싸고 싶지는 않았습니다. 코드는 가능한 한 코드답게 남아 있어야 한다고 생각했거든요. 제가 원한 건 코드는 평소 쓰던 형태로 두고, 그 위에 annotation 정보만 덧붙이는 방식이었습니다.
...
결국 돌고 돌아 찾은 답은 **주석**이었습니다. 이미 Code Hike나 shiki 등의 코드블럭 라이브러리에서도 주석 형태로 사용하기 때문에 익숙한 형태였으며, 주석은 코드 블록 안에 자연스럽게 남겨둘 수 있기 때문에 annotation 정보를 원본 코드와 가장 가까운 위치에서 함께 관리할 수 있는 형식이었기 때문입니다.

이제 남은 건 이 annotation을 파일 안에 어떤 형태로 저장할지 정하는 일이었습니다.

## 주석 체계를 설계하자

주석을 어떻게 설계할지 고민해봤지만 각종 레퍼런스들을 참조한 결과 결론은 단순했습니다. **'어떤 범위에(Range), 어떤 Annotation이(Type), 어떤 속성(Attrs)과 함께'** 붙어 있는지만 완벽하게 표현하면 그만이었으니까요.
...
결과적으로 이 문법은 사람이 메모장에서 읽기에도 크게 어색하지 않았고, 에디터가 다시 파싱하기에도 충분한 정보를 담고 있었습니다. 그렇게 주석의 기본 형태가 정리되었습니다.

## 에디터와 주석의 Collaboration

주석의 형태가 결정되고 나니, 이제 남은 문제는 이 문법을 에디터와 어떻게 호환시킬 것인가였습니다. 에디터에게는 툴팁이나 밑줄을 직접 다룰 수 있는 편집 가능한 인라인 블럭 구조가 필요했지만, 우리가 설계한 주석 문법은 아직 파일 안에 저장되는 텍스트 표현에 불과했기 때문입니다.
...
이 훅들은 \`@keystatic/core\` 바깥에서 주입할 수 있도록, 전역 슬롯을 하나 열어두는 방식으로 연결했습니다.

```ts
globalThis.__KEYSTATIC_MDX_HOOKS__ = { afterMarkdownParse, beforeSerialize };
```

### 통역을 위한 훅 주입과 변환

이 훅이 열리고 나서야 비로소 읽기와 쓰기 양쪽에 변환을 끼워 넣을 수 있게 되었습니다. 하지만 단순히 텍스트 주석과 에디터 노드를 1:1로 갈아 끼우는 방식은 한계가 명확했습니다. 주석은 단순 문자열이지만, 에디터는 드래그 범위와 속성이 포함된 편집 가능한 트리 구조를 원했기 때문입니다.

그래서 저는 이 둘 사이의 간극을 메워줄 중간 포맷, 즉 `Annotation Document`를 따로 두었습니다.
...
flowchart TD
    ...
    style A fill:#dfd,stroke:#333,color:#000
...
먼저 remark 단계에서는 code fence 안의 주석을 읽어, 최종적으로 Shiki가 이해할 수 있는 payload 형태로 정리했습니다.
  ...
  decorations: [
    {
      ...
      properties: {
        ...
        "data-anno-content": "\"모든 검색엔진 크롤러를 대상으로 규칙 적용\""
      }
    }
  ],
  lineDecorations: [],
...
이후 rehype 단계에서는 remark가 미리 심어둔 이 `data-*` 정보를 읽어, 이를 다시 Shiki 하이라이터에 넘긴 뒤 최종 코드 블록 HTML로 치환했습니다. 즉, remark는 주석을 렌더링용 데이터로 정리하는 역할을 맡고, rehype는 그 데이터를 실제 하이라이트 결과로 반영하는 역할을 맡은 셈입니다.

```ts title="src/libs/shiki/rehype-shiki-decoration-render.ts"
const decorations = JSON.parse(code.properties["data-decorations"]);
const lineDecorations = JSON.parse(code.properties["data-line-decorations"]);
const rowWrappers = JSON.parse(code.properties["data-line-wrappers"]);
const meta = JSON.parse(code.properties["data-meta"] ?? "{}");

const hast = highlight(rawCode, lang, meta, {
  decorations,
  lineDecorations,
  rowWrappers,
  allowedRenderTags,
...
처음에는 그저 툴팁 정도만 편하게 쓰고 싶었을 뿐인데, 결과적으로는 코드 블록 전체를 관통하는 annotation 파이프라인을 갖게 된 셈입니다.
...
![](/assets/images/posts/%EC%BD%94%EB%93%9C-%EB%B8%94%EB%9F%AD%EC%97%90-%ED%88%B4%ED%8C%81%EC%9D%84-%EB%9D%84%EC%9A%B0%EA%B3%A0-%EC%8B%B6%EC%97%88%EC%9D%84-%EB%BF%90%EC%9D%B8%EB%8D%B0/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202026-04-09%20%EC%98%A4%EC%A0%84%203.03.26.png)

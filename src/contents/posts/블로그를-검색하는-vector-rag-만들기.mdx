...
이제 문서를 모두 적재했으니 사용자가 쿼리를 입력하고, 답변을 받아오는 코드를 작성해야했습니다. 제 블로그는 Next.js를 사용 중이라 간단하게 API Handler를 사용해 구현했습니다.
...
이렇게 해서 블로그에 RAG를 붙이는 것 자체는 완성됐습니다. 그런데 한 가지 의문이 생겼어요. 과연 이게 제대로 동작하고 있는 걸까요? 그냥 써보면서 느낌으로 판단하기엔 너무 주관적이죠. 다음 편에서는 RAG 성능을 객관적으로 측정하고, chunking 전략을 바꿔가며 개선해보겠습니다.

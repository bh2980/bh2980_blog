...
죄송합니다! 제가 소제목 리스트만 드리고 문단 다듬는 걸 놓쳤네요.

작성하신 내용을 바탕으로, 이전 섹션들에서 보여준 \*\*'설계자의 고민과 해결책'\*\*이라는 톤을 유지하면서 가독성을 높여 다듬어 보았습니다. 특히 `@char`와 `@line`의 차이를 명확히 대조시키는 방향으로 정리했습니다.

---

## 4. 그냥 주석인 줄 알았지? 사실은 직렬화 포맷입니다
...
`// @char Tooltip {0-12} content="모든 검색엔진 크롤러를 대상으로 규칙 적용" User-Agent: * `
...
`// @line collapse export async function generateImageMetadata() { // ... } // @line collapse end `

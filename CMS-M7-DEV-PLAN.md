...
- **진행 상태(2026-09-23 갱신)**: 배치 **1–6 구현·커밋 완료** · R1–R6 리뷰 완료 · SEC-1 독립 검수 **통과** · 배치 8(LEAD-1) 보고서 작성됨 → `CMS-M7-LEAD1-CUTOVER-REPORT.md` · 배치 9(Keystatic 제거) 보류. 전체 게이트: typecheck 0 · tests **685 전부 pass**(실DB env 로드, 0 fail·0 skip) · build exit 0. 남은 차단: **이관 미실행**(운영 DB entries 8건 전부 draft) · 미리보기 DB 초안 경로 · 원격 백업 · 사용자 승인. **v1 미완 항목 하나를 M8로 등록**(§9.18).
...
### 9.18 M7 밖에서 발견한 v1 누락 — M8 등록

M7 범위를 넘는 **v1 기능 누락**을 발견해 `CMS-V1-IMPLEMENTATION-PLAN.md`에 **M8**로 등록했다(구현은 하지 않았다).
...
- M6-ED-1의 “공개 렌더 실패 0”은 레거시 49편만 검사했고 그 49편이 이 컴포넌트를 **0건** 쓰므로 구조적으로 잡히지 않았다. `REGISTERED_JSX_NAMES`는 정의만 되고 `analyze`에서 쓰이지 않으며, 레지스트리↔렌더러 대조 테스트도 없다.
- **M7 전환은 막지 않는다**(49편 0건). 전환 후 새로 쓰는 글에서만 문제가 된다. LEAD-1 보고서 §10에 “v1 미완(전환 비차단)”으로 명시했다.

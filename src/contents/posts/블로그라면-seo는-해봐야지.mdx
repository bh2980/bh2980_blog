...
~~또한 OG 이미지는 정적 이미지 파일을 사용할 수도 있고, 게시글 제목 등을 포함한 이미지를 동적으로 생성할 수도 있습니다. 정적 이미지를 사용할 경우 경로만 지정하면 되고, 동적으로 생성할 경우 별도의 라우트에서 이미지를 만들어 openGraph.images에 연결하면 됩니다.~~

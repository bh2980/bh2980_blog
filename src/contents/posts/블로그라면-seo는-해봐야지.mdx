...
후, 이렇게 길었던 SEO 작업이 끝났습니다. 블로그를 만들면서 꼭 해봐야지라는 마음으로 시작했는데, 생각보다 고려할 내용이 많아서 정리해두길 잘한 것 같습니다.

SEO는 단순히 코드를 몇 줄 추가하는 작업이 아니라, 검색 엔진이 사이트를 어떻게 이해하고 평가하는지에 대한 관점을 정리하는 과정이라는 생각이 들었습니다. 도메인 정리부터 메타데이터, 캐노니컬, 사이트맵, robots 설정까지 하나씩 적용해보면서 웹 서비스의 “겉모습”뿐 아니라 “검색 엔진의 시선”도 함께 고려해야 한다는 걸 체감하게 되었습니다.

당장 눈에 띄는 변화가 나타나지는 않겠지만, 이런 기본 설정들이 쌓여서 장기적으로는 검색 노출과 트래픽에 영향을 줄 것이라 기대해봅니다. 앞으로는 실제 운영 데이터를 보면서 어떤 설정이 어떤 결과로 이어지는지도 기록해보고 싶습니다.

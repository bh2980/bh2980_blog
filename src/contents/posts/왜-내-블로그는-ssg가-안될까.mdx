...
정적과 동적의 개념은 다른 말로 SSG(Static Site Generation)와 SSR(Server Side Rendering)로 말할 수 있습니다. 그리고 이 둘의 차이는 페이지를 언제 생성할 것이냐에 따라 달라집니다.&#x20;
...
즉, Next.js에서 이 페이지는 정적 페이지이고, 이 페이지는 동적 페이지로 빌드되었다라는 표시는 이 페이지를 '언제 생성할 것이냐'를 결정했다는 의미입니다.

...
llms.txt는 <u>아직 공식 표준으로 확정된 규격은 아니며</u>, AI 모델 학습이나 데이터 수집에 사용되는 크롤러를 대상으로 사이트 운영자의 정책을 명시하기 위해 제안된 파일입니다.&#x20;

기존 robots.txt가 검색 엔진 크롤러를 대상으로 한다면, llms.txt는 LLM 관련 크롤러를 대상으로 한다는 점에서 차이가 있습니다. 다만 공식 표준도 아니고 강제성도 없기 때문에 모든 LLM이 준수하는 것은 아닙니다.&#x20;

llms.txt의 관례적 구조는 아래와 같습니다.

...
그 다음으로 끌어낸 것은 상세 페이지에 눌러앉아 있던 `searchParams`였습니다.&#x20;

제 블로그는 카테고리나 태그를 쿼리스트링으로 받아 필터링하는데, 문제는 그 상태를 상세 페이지에서도 끝까지 끌고 갔다는 점이었습니다. 뒤로 가기, 이전 글, 다음 글까지 전부 현재 쿼리를 기준으로 움직이다 보니, 정작 slug만 있으면 되어야 할 상세 페이지가 요청의 맥락까지 책임지고 있었던 것입니다.
...
그래서 상세 페이지는 글 내용과 기본 목록만 들고 있게 두고, 흔들리는 필터 정보(`searchParams`)는 클라이언트 컴포넌트로 따로 유배 보냈습니다.

...
핵심은 검색 방식에 있습니다. 임베딩 모델은 텍스트를 의미를 담은 숫자 벡터로 변환하는데, 이를 활용하면 <u>단어가 정확히 일치하지 않아도 의미가 비슷한 내용을 찾아올 수 있습니다</u>. RAG는 바로 이 벡터 검색을 활용해 관련 문서를 가져오는 방식이죠. 즉, 이를 활용하면 블로그에 쌓인 글들이 단순한 아카이브가 아니라 나와 대화할 수 있는 지식 베이스가 되는 겁니다.
...
먼저 데이터를 저장하기 위해 테이블은 정의했습니다. 청크의 내용과 임베딩 벡터를 함께 저장하고, slug로 원본 게시글을 참조할 수 있도록 했죠.
...
![](/assets/images/posts/%EB%B8%94%EB%A1%9C%EA%B7%B8%EB%A5%BC-%EA%B2%80%EC%83%89%ED%95%98%EB%8A%94-%EB%B2%A1%ED%84%B0-rag-%EB%A7%8C%EB%93%A4%EA%B8%B0/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202026-04-08%20%EC%98%A4%ED%9B%84%2010.25.39.png)

이제 이 스크립트를 실행해서 모든 파일을 읽어와 문서를 임베딩해 벡터로 변환, DB에 저장하면 위처럼 DB에 청크와 벡터 값이 저장되게 됩니다.

### 질의 응답 파이프라인

이제 문서를 모두 저장했으니 사용자가 쿼리를 입력하고, 답변을 받아오는 코드를 작성해야 했습니다. 제 블로그는 Next.js를 사용하고 있기 때문에 Route Handler를 통해 간단하게 POST로 요청하는 API를 구현하였습니다.
...
그리고 마지막으로 검색된 청크를 컨텍스트로 묶어 생성 모델에 전달합니다. 생성 모델로는 openai/gpt-oss-120b:free를 사용했습니다. 이유는 단순한데요. GPT라는 이름값과 무료였기 때문입니다.
...
이렇게 해서 블로그에 RAG를 붙이는 것 자체는 성공했습니다. 물론 깊게 들어가면 복잡해지겠지만, 일단은 시작이니만큼 아주 간단하게 RAG를 구현해볼 수 있었습니다. 서론에서 말했던 것처럼, 드디어 블로그에 쌓인 글들이 단순한 아카이브가 아니라 나와 대화할 수 있는 지식 베이스가 되기 시작한거죠.

그런데 한 가지 의문이 생겼습니다. 일단은 간단하게 만들려고 문서 전체를 하나의 청크로 임베딩했는데, 만약 청크 단위를 좀 더 작게 세분화한다면 성능이 더 좋아질까요? 그렇다면 성능은 어떻게 측정하는 걸까요?

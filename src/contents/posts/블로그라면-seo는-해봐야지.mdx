...
이 둘의 차이는 메타 데이터가 정적인지 동적인지의 차이입니다. 저는 글 본문 페이지에 `/posts/[slug]` 형태의 라우터를 사용하기 때문에 글 본문의 페이지 파일에서 정적으로 메타데이터를 결정할 수 없습니다. 이럴 경우, `metadata` 객체가 아니라 `generateMetadata`를 사용합니다.

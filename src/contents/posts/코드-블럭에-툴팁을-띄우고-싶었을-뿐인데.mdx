...
이 주석을 다시 편집 가능한 구조로 복원하려면, 먼저 에디터와 파일이 서로 다른 구조를 필요로 한다는 사실부터 받아들여야 했습니다.

진짜 어려운 지점은 주석 문법 자체가 아니었습니다. 핵심은 에디터에서 선택한 인라인 블럭과, 저장 시점의 주석 표현 사이를 어떻게 자연스럽게 왕복하게 만들 것이냐였습니다. 사용자는 특정 범위를 드래그한 뒤 버튼을 누를 뿐이지만, 저장될 때는 그 정보가 주석으로 직렬화되고, 다시 열었을 때는 원래의 편집 가능한 구조로 복원되어야 했으니까요.

즉 에디터와 파일은 서로 다른 구조를 필요로 했습니다. 에디터에게는 `Tooltip`이나 `strong`을 직접 다룰 수 있는 인라인 블럭 구조가 필요했고, 파일 입장에서는 그 정보를 코드 블록 안에 남겨둘 수 있는 텍스트 기반 표현이 필요했습니다. 저는 이 차이를 억지로 없애기보다, 중간에서 두 구조를 이어주는 왕복 파이프라인을 두는 편이 더 현실적이라고 판단했습니다.

아래 그림으로 보면 구조는 단순합니다.
...
flowchart LR
    ...
    subgraph Pipeline ["중간 표현"]
        M["annotation<br/>range / type / attrs"]
    end
...
저장할 때는 에디터의 인라인 블럭 정보를 읽어 주석으로 직렬화하고, 다시 불러올 때는 그 주석을 파싱해 에디터가 이해할 수 있는 구조로 복원하는 방식입니다. 말은 간단하지만, 실제로는 Keystatic이 MDX를 어떤 구조로 다루는지 먼저 파악해야 했고, 기본 코드 블록 컴포넌트만으로는 원하는 동작이 나오지 않아 일부는 직접 패치도 해야 했습니다.

이 시점에서 주석의 역할도 분명해졌습니다. 주석은 최종 렌더링을 위한 장식이 아니라, 에디터가 만든 annotation 의미를 파일 안에 보관해 두는 중간 표현이었습니다. 덕분에 사용자는 주석을 직접 의식하지 않아도, 에디터에서는 계속 인라인 블럭을 편집하는 경험을 유지할 수 있었습니다.

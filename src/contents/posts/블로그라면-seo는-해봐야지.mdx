...
robots.txt는 **검색 엔진 크롤러가 어떤 경로를 크롤링 할 수 있는지 제어하는 역할**을 합니다. 사이트의 루트 경로에 위치하고, 크롤러는 이 파일을 먼저 확인한 뒤 페이지를 탐색합니다.&#x20;

예를 들어, 검색 결과에 노출될 필요가 없는 admin 페이지나 API 경로가 존재하는 경우가 존재할 경우, 이를 Disallow에 등록하면 검색엔진 크롤러는 해당 경로를 크롤링하지 않습니다. 다만 이는 접근을 아예 차단하는 설정이 아니라, 크롤러에게 탐색하지 말라고 안내하는 규칙에 가깝습니다.

Next.js에서 robots.txt는 역시 정적인 방법과 동적인 방법으로 적용할 수 있습니다.&#x20;
...
<Tooltip content="src/app">앱의 루트</Tooltip> 경로에 `robots.txt`나 `robots.ts(js)` 파일울 추가하면 되는데요. 정적으로 정의할 경우 txt 파일로 정의하면 되고, 동적인 값이 필요한 경우 `robots` 함수를 활용해 정의 할 수 있습니다.&#x20;

저는 딱히 차이는 없는 것 같아서 정적이지만, 타입 지원을 위해서 robots 함수를 사용했습니다.

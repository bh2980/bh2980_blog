...
robots.txt는 **검색 엔진이 어떤 경로를 크롤링 할 수 있는지 제어하는 역할**을 합니다. 사이트의 루트 경로에 위치하고, 크롤러는 이 파일을 먼저 확인한 뒤 페이지를 탐색합니다.&#x20;

예를 들어 admin 페이지나 api처럼 검색 엔진이 확인하지 않았으면 좋겠다라는 페이지가 존재할 경우, 이를 Disallow에 등록해주면 검색 엔진이 해당 페이지는 크롤링하지 않습니다.
...
앱의 루트(`src/app`) 경로에 `robots.txt`나 `robots.ts(js)` 파일울 추가하면 되는데요. 정적으로 정의할 경우 txt 파일로 정의하면 되고, 동적인 값이 필요한 경우 `robots` 함수를 활용해 정의 할 수 있습니다.&#x20;

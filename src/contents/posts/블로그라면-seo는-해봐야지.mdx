...
저는 동적 방식으로 대표 이미지를 설정해주었는데, `opengraph-image.tsx` 파일을 만들고 Next.js의 `Image` 함수와 `ImageResponse`를 사용한 것을 알 수 있습니다. 대표 이미지의 메타 데이터를 설정하려면, `generateImageMetadata` 함수를 사용하면 됩니다.

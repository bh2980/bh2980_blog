...
이를 해결하기 위해 `reader` 내부에서 직접 동적 요소를 참조하는 대신, 필요한 정보(preview 정보 등)를 파라미터로 전달받도록 수정했습니다. 이렇게 하면 `reader`는 데이터를 읽어온다라는 관심사에만 집중할 뿐 그 외 정보는 파라미터로 받기 때문에 빌드 타임과 런타임을 모두 유연하게 지원하는 함수가 됩니다.

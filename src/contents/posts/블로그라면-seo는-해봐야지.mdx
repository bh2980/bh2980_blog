...
이 둘의 차이는 메타 데이터가 정적인지 동적인지의 차이인데요. 정적 string만을 사용하는 `metadata`와 달리 `generateMetadata`의 경우 `getPost` 함수를 통해 title과 excerpt를 가져와 metadata를 생성하고 있습니다. 이를 통해 블로그 리스트 화면은 그냥 블로그 라는 타이틀을 각 본문은 본문의 타이틀과 요약을 가져와서 설정할 수 있습니다.

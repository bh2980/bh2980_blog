...
에디터가 만든 의미를 파일 안에 보관하려면, 주석은 정확히 어떤 정보를 담아야 할까요? 생각보다 필요한 건 단순했습니다. 어떤 범위에, 어떤 annotation이, 어떤 속성과 함께 붙어 있는지만 표현할 수 있으면 됐습니다.
...
위 예시에서 `@char`는 문자 범위에 적용된다는 뜻이고, `Tooltip`은 annotation의 종류, `{0-12}`는 범위, `content="..."`는 추가 속성입니다. 사용자 입장에서는 드래그 한 범위대로 지정되니, 굳이 range를 수기로 입력할 필요가 없고, 에디터 입장에서는 “특정 범위에 툴팁이 붙어 있다”는 의미면 충분했으며, 파일 입장에서는 그 정보를 텍스트 형태로 보관할 수 있으면 되기 때문에 이 정도 구조면 충분했습니다.

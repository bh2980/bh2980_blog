...
문제는 주석 문법 자체가 아니었습니다. 진짜 어려운 지점은 에디터에서 선택한 인라인 블럭과, 저장 시점의 주석 표현 사이를 자연스럽게 오가게 만드는 일이었습니다. 사용자는 코드 블록 안의 특정 범위를 드래그한 뒤 **strong**이나 **Tooltip** 버튼을 누를 뿐이지만, 저장될 때는 그 정보가 주석으로 바뀌고, 다시 열었을 때는 원래의 편집 가능한 구조로 복원되어야 했습니다.

즉, 에디터와 파일은 서로 다른 구조를 필요로 했습니다. 에디터에게는 Tooltip이나 strong을 직접 다룰 수 있는 인라인 블럭 구조가 필요했고, 파일 입장에서는 그 정보를 코드 블록 안에 남겨둘 수 있는 직렬화된 표현이 필요했습니다. 저는 이 둘을 억지로 하나로 맞추기보다, 서로 다른 표현으로 인정한 뒤 중간에서 변환하는 편이 더 현실적이라고 판단했습니다.

```mermaid
flowchart LR
    subgraph Editor ["에디터 (Visual Context)"]
        E["선택 영역 + 인라인 블럭<br/>(strong / Tooltip)"]
    end

    subgraph Pipeline ["왕복 파이프라인 (The Interpreter)"]
        direction LR
        M["중간 표현 (Annotation)<br/>range / type / attrs"]
    end

    subgraph Storage ["저장소 (Markdown)"]
        F["코드 블록 + 주석 데이터"]
    end
    ...
    %% 불러오기 흐름 (Parse & Rehydrate)
    F -->|"3. 데이터 추출 (Parse)"| M
    M -->|"4. UI 복원 (Rehydrate)"| E

    style E fill:#f9f,stroke:#333,stroke-width:2px
    style F fill:#bbf,stroke:#333,stroke-width:2px
    style M fill:#fff,stroke:#333,stroke-dasharray: 5 5
```

그래서 저장할 때는 인라인 블럭 정보를 읽어 주석으로 직렬화하고, 다시 불러올 때는 그 주석을 파싱해 편집 가능한 구조로 복원하는 파이프라인을 만들게 되었습니다. 말은 간단하지만, 실제로는 Keystatic이 내부에서 MDX를 어떤 구조로 다루는지 먼저 파악해야 했고, 기본 코드 블록 컴포넌트만으로는 원하는 동작이 나오지 않아 일부는 직접 패치도 해야 했습니다. 결국 제가 만든 것은 단순한 주석 파서가 아니라, 에디터와 파일 사이를 오가는 왕복 파이프라인에 가까웠습니다.

이 시점에서 주석의 역할도 분명해졌습니다. 주석은 최종 렌더링을 위한 장식이 아니라, 에디터가 만든 의미를 파일 안에 보관해 두는 중간 표현이었습니다. 이제 남은 고민은 하나였습니다. 이 중간 표현을 어떤 형태로 설계해야 다시 읽어 왔을 때 무리 없이 원래 구조로 복원할 수 있을까?

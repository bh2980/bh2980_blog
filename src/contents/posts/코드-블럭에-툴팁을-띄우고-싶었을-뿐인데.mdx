...
주석 문법을 어떻게 설계할지 고민하기 전에, 먼저 풀어야 할 문제는 에디터였습니다. 정확히 말하면, Keystatic의 기본 코드 블록이 어떤 구조로 MDX를 저장하고 다시 복원하는지부터 이해해야 했습니다. 제가 원한 건 코드 블록 안에서도 `strong`이나 `Tooltip` 같은 인라인 블럭을 일반 본문처럼 다루는 것이었는데, 기본 코드 블록 컴포넌트는 애초에 그런 편집 모델을 상정하고 있지 않았기 때문입니다.
...
그리고 나서야 다음 질문이 구체적으로 보이기 시작했습니다. 에디터와 파일 사이를 오가는 이 왕복 흐름을 유지하려면, 주석은 어떤 형태로 설계되어야 할까?

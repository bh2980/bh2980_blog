...
flowchart TD
    ...
    style C fill:#fff4dd,stroke:#333
    style D fill:#fff1f1,stroke:#ff9494,stroke-width:2px,stroke-dasharray: 5 5
...
주석의 형태가 결정되고 나니, 이제 남은 문제는 이 문법을 에디터와 어떻게 호환시킬 것인가였습니다. 에디터에게는 Tooltip이나 strong을 직접 다룰 수 있는 편집 가능한 인라인 블럭 구조가 필요했지만, 우리가 설계한 주석 문법은 아직 파일 안에 저장되는 텍스트 표현에 불과했기 때문입니다.
...
먼저 remark 단계에서는 code fence 안에 들어 있는 주석을 읽어 중간 포맷으로 파싱한 뒤, 이를 다시 Shiki가 이해할 수 있는 payload로 바꿨습니다.
...
처음에는 그저 Tooltip과 strong 정도만 편하게 쓰고 싶었을 뿐인데, 결과적으로는 코드 블록 전체를 관통하는 annotation 파이프라인을 갖게 된 셈입니다.

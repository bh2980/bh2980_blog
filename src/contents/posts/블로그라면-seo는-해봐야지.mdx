...
export const metadata: Metadata = {
	...
	description: "개발하면서 배운 것들과 경험을 기록합니다.",
	// @line plus
	// @char Tooltip {26-33} content="root에서 metadataBase를 설정하였으므로 상대 경로로 지정 가능"
...
Next.js(App Router)에서는 `src/app` 디렉터리에 `robots.txt` 또는 `robots.ts` 파일을 추가하면, 빌드 결과로 사이트 루트에 해당 파일이 생성됩니다.
...
<Callout variant="note">
  사이트 전체에 대한 설정을 제어하는 robots.txt와 다르게 메타 데이터에서 robots 옵션을 통해 **페이지별 색인 여부와 링크 추적 여부를 설정**할 수 있습니다.
  ...
  예를 들어 위와 같이 게시글을 찾지 못했을 경우에 Not Found 페이지를 보여주는데, robots를 설정해 현재 페이지를 <Tooltip content="index: false">색인에 추가하지는 않되</Tooltip>, <Tooltip content="follow: true">페이지 내부에 있는 링크는 따라가도록</Tooltip> 설정할 수 있습니다.
</Callout>

### 5. sitemap.xml

robots.txt가 크롤러에게 페이지의 크롤링 정책을 안내하는 역할이라면, sitemap.xml은 사이트 내에서 크롤링 및 색인이 이루어지길 원하는 URL 목록을 검색엔진에 전달하는 역할을 합니다. 즉, robots.txt와는 상호보완적 관계라고 할 수 있겠네요.
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
	...
	const memoSitemap = memos.list.map<MetadataRoute.Sitemap[number]>((memo) => ({
		url: new URL(`https://bh2980.dev/memos/${memo.slug}`).toString(),
...
저는 Next.js에서 제공하는 sitemap 함수에 게시글과 메모 리스트를 가져와 넣어주는 방식으로 작성했는데요. 이렇게 하면 게시글이 추가될 때마다 사이트맵이 함께 갱신되므로, 수동으로 관리할 때 발생할 수 있는 누락 가능성을 줄일 수 있습니다.

<Callout variant="info">
  `lastModified`에 들어가는 날짜의 형식은 ISO 8601 형식을 따라야합니다.
</Callout>
...
<Callout variant="info">
  하나의 사이트맵에 들어갈 수 있는 URL의 개수는 최대 50,000개입니다. 때문에 50,000개 이상의 URL을 사이트맵에 담으려면 [`getSitemaps`](https://nextjs.org/docs/app/api-reference/file-conventions/metadata/sitemap#generating-multiple-sitemaps) 함수를 이용해서 사이트맵을 여러 개로 분할해 만들 필요가 있습니다.
</Callout>

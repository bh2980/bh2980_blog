...
① 실DB 계약 미검증 → **해소**(685/685). ② 미리보기 DB 초안 경로 → 미해결(승인 조건). ③ R3–R6 완료, SEC-1 재검수 통과 → **해소**. ④ 이관 무결성 대조 → 미실행(운영 DB에 콘텐츠 0건). ⑤ 원격 백업 → 미해결.

### 9.18 M7 이후 순서 재정렬 — M8 표현 계약, M9 이관과 전환

**사용자 지적(2026-09-23):** “컴포넌트가 만들어지지도 않았는데 DB를 배포한다느니… 마일스톤 순서가 엉망” — 맞다. M7 시점의 계획은 **컴포넌트와 저장 형식이 없는 상태에서 전환을 먼저** 하는 순서였다. `CMS-V1-IMPLEMENTATION-PLAN.md`를 다시 짰다.

**발견한 누락**

- §4.4의 `TextAlign`·`Image`·`ContentLink`는 F16(§6.2)·F18(§4.3)의 **v1 기능**이고, F18 수용 기준은 “공개 PC·모바일에서 컨테이너를 넘지 않고 비율이 보존된다”까지 요구한다(`CMS-SPEC.md:644`).
- 그런데 **공개 렌더러 구현 태스크가 M1~M7 어디에도 없다.** M1-ED-1은 serialize의 이름 유지, M3-ED-2는 편집기 삽입까지였다. 계획서는 공개 렌더를 “기존 블로그 경로 재사용”으로 전제했지만 그 컴포넌트는 저장소 역사상 한 번도 없었다(`origin/main`도 동일).
- M6-ED-1의 “공개 렌더 실패 0”은 레거시 49편만 검사했고 그 49편이 이 컴포넌트를 **0건** 쓰므로 구조적으로 잡히지 않았다. `REGISTERED_JSX_NAMES`는 정의만 되고 `analyze`에서 쓰이지 않는다.

**사용자 결정: 저장 형식을 remark directive로**

- `:[`만 검사하면 안 된다 — `[label]`은 선택이고 이름에 숫자가 허용된다(micromark 문서). `http://`는 안전하다(이름이 `/`로 시작할 수 없다).
- 레거시 49편 **본문**에서 오인 패턴 **실측 2건**: `openai/gpt-oss-120b:free`, `1:1`(코드 블록·frontmatter 제외, `.pi/directive-risk.mjs`).
- 진짜 위험은 빈도가 아니라 **무음**이다: “If directives are not handled, they do not emit anything”, 닫는 펜스 누락은 문서 끝까지 삼킨다. 그래서 M8-TW-1이 미등록 지시자를 **거부**한다.

**새 순서**

```text
M7 (완료)  공개 조회 기반 — 배치 1~6 구현·검수
  → M8     표현 계약 정비: ① directive 읽기(추가형) ② 누락 컴포넌트 3종 ③ 거부·대조 테스트 ④ 43편 변환 ⑤ serializer·에디터 쓰기 전환
  → M9     이관과 전환: ① 미리보기 DB 초안 경로 ② 이관 실행+주소 대조 ③ 전후 HTML 대조 ④ 승인 보고서 ⑤ 플래그 전환+재배포 ⑥ Keystatic 제거
```

...
1. `object`, `Object`, `{}` 타입
   ...
   `Object` : 자바스크립트의 전역 Object 생성자(래퍼) 타입, 매우 넓다. 필터로 사용하지 적절치 않다.
...
3. readonly <Tooltip content="배열과 튜플">리스트</Tooltip>
   ...
   readonly 리스트 타입은 mutable 리스트 타입보다 제약이 적어 더 큰 범위(집합)에 해당한다. 따라서 mutable 리스트는 readonly 리스트에 할당 가능하다.

...
| M4 | 일괄 작업 | §12.1-4 | 미디어 라이브러리(DONE), 일괄 작업(태그·카테고리·폴더·상태)이 사용처·삭제 규칙을 지킨다 |
| M5 | 복제·템플릿·저작 확장 | §12.1-4 | 복제, 본문 템플릿 독립 CRUD 및 적용, 관계 선택기와 record 폼, 필드/블록 확장 예제가 검증된다 |
| M6 | 이전·내보내기 검수 | §12.1-4 | 내보내기·시험 이전·전편 왕복 검수가 전환 차단 이슈를 분류한다 |
| M7 | 블로그 연결 | §12.1-5 | 공개 조회가 DB 공개본만 쓰고, 글별 SEO 메타를 출력하며, 기존 49편 검수 보고서가 있고, Keystatic 제거 준비와 전환 승인을 사용자에게 맡긴다 |

각 Milestone 병합 후 Reviewer가 통합 브랜치를 검수한다. 운영 데이터 이전과 공개 전환은 M7 보고서 이후 **사용자 승인**이 따로 필요하다.
...
종료 조건: F09 일괄 작업(태그·카테고리·폴더·상태)과 F14 라이브러리. 관심사는 목록 운영 효율 하나로 한정한다. 복제·템플릿은 M5, 이전·내보내기는 M6에서 다룬다.
...
- **선행:** M4-BE-1, M2-FE-1
- **담당:** FE
- **완료 조건:** 현재 페이지 선택만. 체크박스 선택 후 태그·카테고리·폴더·보관·휴지통·발행 일괄 실행. 일부 실패 시 성공/실패 개별 표시.
- **검증:** 브라우저
...
종료 조건: F13 복제, 본문 템플릿 독립 CRUD(F13 확장) 및 적용, F07 확장 예제, §6.1 관계 선택기와 §5.2 record 폼, collections.ts 단일 레지스트리. 관심사는 저작 확장 하나로 한정한다.

### M5-BE-1 복제 API (ex-M4-BE-2)

- **목적:** F13 콘텐츠 복제.
- **주요 내용:** `POST /api/cms/v1/entries/:id/duplicate`. 최신 초안의 본문·필드·관계 전체를 새 ID로 복사. 제목은 `원본 제목 (복사)`로 시작하고 slug는 비워 둔다. 발행 상태·일정·공개본·생성/수정일은 비복사. 원본과 같은 가상 폴더의 새 초안이 되며, 미디어 재업로드 없이 기존 자산을 참조한다.
- **선행:** M4-BE-1
- **담당:** BE
- **완료 조건:** 복제 글이 초안이고 원본 공개본/일정과 무관하다. 계약 테스트 통과.
- **검증:** API 테스트

### M5-BE-2 본문 템플릿 독립 백엔드 (Oracle 자문 방안 B)

- **목적:** F13 템플릿 관리. entries 인프라 오염(발행·스케줄·미디어삭제가드·검색·일괄 작업 누수)을 원천 차단하기 위해 독립 `body_templates` 리소스로 분리.
- **주요 내용:**
  - 테이블: `body_templates (id uuid primary key, name text not null, for_collection text not null, mdx text not null, version integer not null default 1, created_at timestamptz not null, updated_at timestamptz not null)`. `for_collection`은 본문이 있는 `post`, `memo`만 허용.
  - 기본 템플릿 2종 1회성 seed: 알고리즘 풀이 메모(`## 문제 / ## 풀이`), Type Challenge 풀이 메모(`### 질문 / ### 풀이` + 코드 틀). 재부팅 시 삭제된 템플릿이 부활하지 않도록 1회성 마커/마이그레이션 처리. DB가 유일한 진실 원본.
  - CRUD API: `GET/POST /api/cms/v1/templates`, `GET/PATCH/DELETE /api/cms/v1/templates/:id`. 낙관적 락(`expectedVersion`), admin 인증 및 Same-Origin 적용.
  - 템플릿 적용은 순수 본문(MDX) snapshot 복사이며, template ID를 글에 저장하지 않는다. 템플릿 수정·삭제는 이미 작성된 글에 일체 영향 없음.
- **선행:** M5-BE-1
- **담당:** BE
- **완료 조건:** templates CRUD API 동작, 동시 수정 충돌(409) 방지, 1회성 seed 검증. 계약 테스트 통과.
- **검증:** API 계약 테스트

### M5-FE-1 에디터 툴바 템플릿 적용 버튼 (M4-FE-2에서 분리)

- **주요 내용:** 에디터 상단 툴바에 `템플릿` 버튼. 클릭 시 현재 글의 collection(`post` 또는 `memo`)에 맞는 템플릿만 목록 표시 (`GET /api/cms/v1/templates?forCollection=...`). 선택 시 확인 다이얼로그("현재 본문이 교체됩니다") 후 **본문(MDX)만 교체**하고 메타데이터(태그/카테고리)는 손대지 않음. 기존 글 자동저장(`triggerSave`)으로 persisted.
- **선행:** M5-BE-2, M3-FE-1
- **담당:** FE
- **완료 조건:** 템플릿 적용 후 저장·재열기 시 본문 유지, 메타데이터 보존. 빈 본문일 때는 확인 없이 즉시 적용, 내용이 있을 때는 확인 취소 시 본문 유지. 다른 collection 템플릿은 목록에 안 뜸.
- **검증:** 브라우저

### M5-FE-3 템플릿 관리 UI

- **주요 내용:** `/admin/templates` 화면. 사이드바 네비게이션에 `템플릿` 메뉴 추가. 템플릿 목록(이름, 대상 컬렉션, 수정일), 생성 다이얼로그/페이지, 편집, 삭제(확인 모달). 편집 화면은 `CmsEditor` 알맹이 컴포넌트만 재사용(slug·인스펙터·발행·예약 제외, 명시적 저장 버튼).
- **선행:** M5-BE-2
- **담당:** FE
- **완료 조건:** 템플릿 생성·수정·삭제가 목록에 즉시 반영된다. 에디터 툴바 드롭다운에 새로 만든 템플릿이 노출된다.
- **검증:** 브라우저

### M5-FE-2 관계 선택기와 record 폼 (갭 분석 반영)

- **목적:** §6.1 관계 선택기, §5.2 record workflow. 외부 CMS(Keystatic/Strapi/WP/Ghost) 대비 확인된 P0 갭.
- **주요 내용:** 에디터 인스펙터에 태그/카테고리/모음집/관련글 관계 picker. 제목·이름으로 검색하고 컬렉션/상태/slug를 함께 표시, 다중 선택 순서 유지. 자유 텍스트 tags 입력을 tagIds 선택으로 교체. 태그/카테고리/모음집 record 관리 화면(생성·이름 변경, 휴지통 규칙 준수). slug 변경 시 "기존 주소는 308로 유지됩니다" 안내 문구.
- **선행:** M2-BE-3, M3-FE-1
- **담당:** FE
- **완료 조건:** picker 선택이 metadata ID로 저장·재열기된다. record 이름 변경이 기존 글 표시명에 반영된다(참조는 ID 기준).
- **검증:** 브라우저 + API

### M5-ED-1 필드·블록 확장 예제 및 collections.ts 단일 레지스트리 (F07, ex-M4-ED-1)

- **목적:** 코어를 수정하지 않고 필드/블록을 붙이는 경로를 문서와 최소 예제로 닫고, 분산된 하드코딩을 단일 레지스트리로 모은다.
- **주요 내용:**
  - `collections.ts` 단일 레지스트리: types/service/api/meta에 흩어진 컬렉션 정의(이름, 필드 규칙, 관계 추출 규칙)를 단일 파일로 정리 (동작 변경 없음, v2 사용자 정의 스키마 기초).
  - 확장 예제: 색상 필드, slug 보조 버튼(`renderDefault`), 링크 object 필드, 코드 fence `defineBlock` 연결. 기존 `fields.ts`/`blocks.ts` API 사용. 마켓/로더 없음.
- **선행:** M3-FE-1, M1-ED-1
- **영향 파일:** `src/cms/core` 예제 등록, `docs/cms/extensions.md` (신규 문서만)
- **담당:** ED + FE (입력 컴포넌트)
- **완료 조건:** 예제 필드가 저장·재열기된다. React 함수가 `/meta` JSON에 안 실림 (기존 contracts.test). 단일 레지스트리가 기존 스키마 검증을 완전히 대체.
- **검증:** 계약 테스트 + 수동 저장/재열기

### M5-RV-1 Milestone 5 검수

- **담당:** RV
- **선행:** M5-BE-1, M5-BE-2, M5-FE-1, M5-FE-3, M5-FE-2, M5-ED-1

---

## M6. 이전·내보내기 검수

종료 조건: §11.3 1–6의 **시험** 이전 도구와 §11.4 내보내기. 관심사는 전환 예행연습 하나로 한정한다. 운영 전환은 하지 않는다.

### M6-BE-1 내보내기 API (ex-M4-BE-3)
...
### M6-BE-2 기존 콘텐츠 시험 가져오기 (쓰기 전 검사 포함, ex-M4-BE-4)
...
### M6-ED-1 이전 본문 왕복 검수 러너 (ex-M4-ED-2)
...
### M6-RV-1 Milestone 6 검수

- **담당:** RV
- **선행:** M6-BE-1, M6-BE-2, M6-ED-1

---

## M7. 블로그 연결과 전환 준비

종료 조건: 공개 페이지가 DB 공개본만 보여주고, 보관/휴지통/초안이 RSS·sitemap·OG·목록에 없다. 글별 SEO 메타를 출력한다. 전환 보고서가 있다. Keystatic **제거는 사용자 전환 승인 후** 별 Task. 이 Milestone에서 제거 준비와 플래그만 한다.
...
### M7-BE-1 Postgres ContentRepository
...
### M7-BE-2 공개 라우트 동적화 (RSS, sitemap, 목록, 상세)
...
### M7-FE-2 SEO 메타 필드와 공개 메타 렌더 (갭 분석 반영)

- **목적:** 공개 SEO 품질. Ghost/WP 표준 대비 확인된 P1 갭. 대표 이미지는 범위 밖(연기됨).
- **주요 내용:** 글별 meta title/description override(미입력 시 title/summary 폴백), canonical URL, OG/Twitter 카드 제목·설명·이미지(이미지는 미디어 라이브러리에서 선택). M7-BE-2 공개 경로의 head 렌더에 반영. custom canonical이 있는 글은 sitemap에서 제외(Ghost 규칙).
- **선행:** M7-BE-1, M7-BE-2
- **담당:** FE
- **완료 조건:** 공개 페이지 head에 title/meta/OG/canonical이 출력되고 미입력 시 폴백이 동작한다.
- **검증:** 브라우저 + head 대조

### M7-BE-3 공개 HTTP `/public/entries`
...
8. M3-BE-1 발행/예약  
9. M6-BE-2 시험 이전 + M6-ED-1 전편 왕복  
10. M7-BE-1/2 공개 조회 전환  
11. M7-LEAD-1 사용자 전환 승인  
12. M7-BE-5 Keystatic 제거  
...
| 기존 정적 생성 + 신규 글 무재배포 | 명세 §11.1. 캐시가 보관 글을 남기면 전환 실패 | M7-INV-1 |
| 기존 상대 경로 이미지 vs R2 mediaId | 자동 다운로드 금지. 주소 유지가 우선 | M0-INV-3, M6-BE-2 |
...
## 8. v2 후보: 사용자 정의 스키마 (추후 개편, v1 범위 밖)

목표: 사용자가 컬렉션·필드를 코드와 web(`/admin`) 양쪽에서 자유롭게 추가·수정. v1 완료 후 별도 마일스톤으로 설계·착수한다. 명세 개정 필요.

- **1단계(v1 내):** 코드 레지스트리 단일화. 타입·검증·참조규칙·`/meta` 노출이 하드코딩 3곳 대신 `collections.ts` 한 곳을 본다. M5-ED-1 범위.
- **2단계(v2):** 스키마를 DB로. `collection_definitions` 테이블에 필드 정의 저장, 검증 로직이 레지스트리를 읽게 전환. 시스템 컬렉션(post/memo + slug/주소 불변식)은 잠금, 커스텀만 web 편집 허용.
- **3단계(v2):** web 스키마 빌더 + 동적 폼. 인스펙터가 레지스트리를 읽어 폼 자동 생성.
- **둑:** 발행글과 호환 안 되는 변경(필수 추가·타입 변경)은 경고 + 마이그레이션 확인 없이 불가. 런타임 검증 필수(TS는 빌드 타임이라 소용없음).
- **연기됨:** 대표 이미지(M7-FE-2 범위 밖, 필요 시 v2에서 재검토).

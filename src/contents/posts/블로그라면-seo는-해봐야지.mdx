...
OG 태그는 robots.txt나 sitemap.xml처럼 검색 엔진의 크롤링이나 색인을 직접 제어하는 요소는 아니지만, 공유 시점에 어떤 제목과 설명, 이미지를 보여줄지를 결정하기 때문에 클릭률과 콘텐츠 확산에 영향을 주는 요소입니다. 즉, 검색 순위를 직접 올려주는 장치는 아니지만, 외부 유입과 재공유를 통해 간접적으로 SEO에 기여한다고 볼 수 있습니다.

Next.js에서는 메타데이터의 openGraph 속성을 정의해 OG 정보를 설정할 수 있습니다. 저는 루트 레이아웃에서 기본 값을 설정해두고, 게시글 상세 페이지에서는 generateMetadata를 사용해 각 게시글의 제목과 요약을 OG에 반영하는 방식으로 구성했습니다.

먼저 루트 레이아웃에서 공통으로 사용할 기본 OpenGraph 설정을 정의할 수 있습니다.

```ts
[루트 레이아웃(src/app/layout.tsx)에서 metadataBase, openGraph, twitter 기본 값을 설정하는 코드]
```

게시글 상세 페이지(/posts/\[slug])처럼 동적인 라우트에서는 generateMetadata를 활용해 게시글 데이터를 기반으로 OG 정보를 생성합니다. 이렇게 하면 게시글이 추가되거나 수정될 때마다 공유 카드 정보도 함께 갱신됩니다.

```ts
[게시글 상세 페이지(src/app/posts/[slug]/page.tsx)에서 generateMetadata로 post.title, post.excerpt, openGraph, twitter를 반환하는 코드]
```

또한 OG 이미지(og:image)는 정적 이미지 파일을 사용할 수도 있고, 게시글 제목 등을 포함한 이미지를 동적으로 생성할 수도 있습니다. 정적 이미지를 사용할 경우 경로만 지정하면 되고, 동적으로 생성할 경우 별도의 라우트에서 이미지를 만들어 openGraph.images에 연결하면 됩니다.



\[정적 OG 이미지(openGraph.images)를 설정하는 코드]

\[동적 OG 이미지를 생성하는 라우트 또는 Next.js OG Image 기능을 사용하는 코드]



(카카오톡이나 디스코드에서 게시글 링크를 공유했을 때 OG 카드가 정상적으로 표시되는 화면 스크린샷)

...
Next.js에서는 메타데이터의 `openGraph` 속성을 정의해 OG 정보를 설정할 수 있습니다만, 설정하지 않을 시 기본적으로 메타데이터의 title과 description을 사용하게 되어있습니다. 그래서 저는 별도로 설정해주진 않았습니다.
...
위는 동적 방식으로 대표 이미지를 설정한 코드인데요. `opengraph-image.tsx` 파일을 만들고 Next.js의 `Image` 함수와 `ImageResponse`를 사용한 것을 알 수 있습니다. 대표 이미지의 메타 데이터를 설정하려면, `generateImageMetadata` 함수를 사용하면 됩니다.

이처럼 동적 방식으로 설정할 경우 컴포넌트 코드처럼 작성할 수 있다는 특징이 있습니다. 하지만 모든 style 문법을 지원하는 것은 아니므로 스타일이 적용되는지 유의하며 사용해야 합니다.
...
그렇다면 만들어진 OpenGraph를 어떻게 확인하느냐
